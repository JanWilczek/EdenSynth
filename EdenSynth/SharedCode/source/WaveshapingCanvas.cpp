...
void WaveshapingCanvas::setTransferFunction(
...
void WaveshapingCanvas::drawPlotAxes(Graphics& g) {
  ...
  Line<float> leftVerticalPlotLine(getWidth() / 4.f, 0.f, getWidth() / 4.f,
                                   static_cast<float>(getHeight()));
  constexpr float dashes[] = {7.f, 5.f};
  g.drawDashedLine(leftVerticalPlotLine, dashes, 2, 0.5f);
  leftVerticalPlotLine.applyTransform(
      AffineTransform(1.f, 0.f, 0.f, 0.f, 1.f, 0.f)
          .translated(getWidth() / 2.f, 0.f));
  g.drawDashedLine(leftVerticalPlotLine, dashes, 2, 0.5f);

  Line<float> upperHorizontalPlotLine(0.f, getHeight() / 4.f,
                                      static_cast<float>(getWidth()),
                                      getHeight() / 4.f);
  g.drawDashedLine(upperHorizontalPlotLine, dashes, 2, 0.5f);
  upperHorizontalPlotLine.applyTransform(
      AffineTransform(1.f, 0.f, 0.f, 0.f, 1.f, 0.f)
          .translated(0.f, getHeight() / 2.f));
  g.drawDashedLine(upperHorizontalPlotLine, dashes, 2, 0.5f);
}
