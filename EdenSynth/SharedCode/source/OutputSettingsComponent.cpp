...
OutputSettingsComponent::OutputSettingsComponent(AudioProcessorValueTreeState& valueTreeState)
	: _volume(Slider::SliderStyle::Rotary, Slider::TextEntryBoxPosition::NoTextBox)
{
	...
	_volumeAttachment = std::make_unique<SliderAttachment>(valueTreeState, "output.volume", _volume);
}
...
void OutputSettingsComponent::resized()
{
	...
	_volume.setBounds(getWidth() / 2 - knobDim / 2, _volumeLabel.getY() + _volumeLabel.getHeight(), knobDim, knobDim);
}
