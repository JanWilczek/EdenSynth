...
FilterControlsComponent::FilterControlsComponent(AudioProcessorValueTreeState& valueTreeState)
	...
	, _contourAmount(Slider::SliderStyle::Rotary, Slider::TextEntryBoxPosition::NoTextBox)
{
	...
	_contourAmountAttachment = std::make_unique<SliderAttachment>(valueTreeState, "filter.contourAmount", _contourAmount);
	...
	_passbandAttenuationAttachment = std::make_unique<ComboBoxAttachment>(valueTreeState, "filter.passbandAttenuation", _passbandAttenuation);
}
...
void FilterControlsComponent::resized()
{
	...
	_resonanceLabel.setBounds(_cutoffLabel.getX() + cellSize, _cutoffLabel.getY(), cellSize, labelHeight);
	_resonance.setBounds(_cutoff.getX() + cellSize, _cutoff.getY(), knobDim, knobDim);

	_contourAmountLabel.setBounds(_resonanceLabel.getX() + cellSize, _resonanceLabel.getY(), cellSize, labelHeight);
	_contourAmount.setBounds(_resonance.getX() + cellSize, _resonance.getY(), knobDim, knobDim);

	_passbandAttenuationLabel.setBounds(_contourAmountLabel.getX() + cellSize, _contourAmountLabel.getY(), cellSize, labelHeight);
	_passbandAttenuation.setBounds(_passbandAttenuationLabel.getX(), cellSize / 2, cellSize, labelHeight);
}
