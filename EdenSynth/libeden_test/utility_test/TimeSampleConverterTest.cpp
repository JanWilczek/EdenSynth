...
namespace libeden_test
{
	TEST(TimeSampleConverterTest, SecondsToSamples)
	{
		...
		for (auto i = 0; i < 3; ++i)
		{
			const auto result = eden::utility::TimeSampleConverter::timeToSamples(durationsInS[i], sampleRate);
	...
	TEST(TimeSampleConverterTest, MillisecondsToSamples)
	{
		...
		constexpr std::chrono::milliseconds durationsInS[] = { 500ms, 2000ms, 10ms, 20ms };
	...
	TEST(TimeSampleConverterTest, SamplesToTime)
	{
		...
		constexpr std::chrono::milliseconds millisecondsResults[] = { 10ms, 1000ms, 5ms, 2000ms };

		for (auto i = 0; i < 4; ++i)
		{
			const auto seconds = eden::utility::TimeSampleConverter::samplesToSeconds(samples[i], sampleRate);
			EXPECT_EQ(seconds, secondsResults[i]);

			const auto milliseconds = eden::utility::TimeSampleConverter::samplesToMilliseconds(samples[i], sampleRate);
