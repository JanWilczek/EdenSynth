...
protected:
  ...
  void SetUp() override {
    fillChannel(1.f);
    _data = GetParam();
    eden::ADBDRParameters envelopeParameters(
        _data.attackTime, eden::EnvelopeSegmentCurve::Exponential,
        ...
        _data.breakLevel);
    _envelope = std::make_unique<eden::synth::envelope::ADBDR>(
        SAMPLE_RATE, envelopeParameters);
  }
  ...
  /// <param name="toSample">after execution this parameter is the index in
  /// buffer of the last processed sample</param> <returns>the number of
  /// processed samples</returns>
  unsigned processSamplesRange(const unsigned fromSample, unsigned& toSample) {
    ...
    _envelope->applyInRange(_channel.get(), fromSample,
                            initialSamplesToProcess);
  ...
  void shapeTest(int startSample, int endSample) {
    ...
    const unsigned middleSample =
        static_cast<unsigned>((endSample + startSample) / 2);
    ...
    EXPECT_NEAR(middleSampleLevel, (startSampleLevel + lastSampleLevel) / 2,
                0.2);
  }
...
TEST_P(ADBDRTest, AttackDecayTest) {
  ...
  const auto attackSamples = eden::utility::TimeSampleConverter::timeToSamples(
                                 _data.attackTime, SAMPLE_RATE) +
                             1;
  ...
  const auto decay1Samples = eden::utility::TimeSampleConverter::timeToSamples(
      _data.decay1Time, SAMPLE_RATE);
  ...
  const auto endSample = findFirstSample(breakSample, [](float sample) {
    return std::abs(sample - float(0.0)) < 1e-6;
  });
...
TEST_P(ADBDRTest, ReleaseTest) {
  ...
  const auto attackDecay1Samples =
      eden::utility::TimeSampleConverter::timeToSamples(
          _data.attackTime + _data.decay1Time, SAMPLE_RATE) +
      1;
  ...
  const auto releaseSamples =
      eden::utility::TimeSampleConverter::timeToSamples(_data.releaseTime,
                                                        SAMPLE_RATE) +
      10;  // small offset is given, because exponential gain is not precise.
  ...
  // Handle case when release is very short (only one apply iteration) and when
  // it is very long (we can start from sample 0 then).
  ...
  auto foundEndSample = findFirstSample(
      releaseStartOnChannel,
      [](float sample) { return std::abs(sample - float(0.0)) < 1e-6; });
