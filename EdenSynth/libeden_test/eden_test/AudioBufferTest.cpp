...
namespace libeden_test
{
	...
	TEST(AudioBufferTest, Fill)
	{
		...
		for (size_t k = 0; k < testValues.size(); k++)
		{
			...
			for (auto i = 0; i < testBuffer.getNumChannels(); ++i)
			{
				...
				{
					EXPECT_FLOAT_EQ(testBuffer.getArrayOfReadPointers()[i][j], static_cast<float>(testValues[k]));
				}
			}
		}
	}
	...
	TEST(AudioBufferTest, AddSample)
	{
		...
		{
			testBuffer.addSample(0, i, static_cast<float>(std::sin(2 * FREQUENCY1 * eden::math_constants::PI * i / SAMPLE_RATE)));
			testBuffer.addSample(0, i, static_cast<float>(std::sin(2 * FREQUENCY2 * eden::math_constants::PI * i / SAMPLE_RATE)));
		}
		for (auto j = 0u; j < testBuffer.getNumSamples(); ++j)
		{
			const auto expectedValue = static_cast<float>(std::sin(2 * FREQUENCY1 * eden::math_constants::PI * j / SAMPLE_RATE) + std::sin(2 * FREQUENCY2 * eden::math_constants::PI * j / SAMPLE_RATE));
