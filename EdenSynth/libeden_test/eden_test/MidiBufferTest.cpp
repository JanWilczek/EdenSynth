...
namespace libeden_test
{
	...
	{
		const eden::MidiMessage message(0, eden::MidiMessage::MidiMessageType::Aftertouch);
		eden::MidiBuffer buffer;

		buffer.addEvent(message, 2);

		ASSERT_EQ(buffer.getNumEvents(), 1);
		ASSERT_EQ(buffer.begin()->message.getType(), eden::MidiMessage::MidiMessageType::Aftertouch);
	}
	...
	TEST(MidiBufferTest, MultipleNoteOnMessages)
	{
		...
		{
			EXPECT_EQ(iterator->message.getType(), eden::MidiMessage::MidiMessageType::NoteOn);
