...
/// <summary>
/// Class responsible for changing parameters of signal generators belonging to
/// all voices.
/// </summary>
...
public:
  ...
  /// <summary>
  /// Creates a new realtime oscillator source with a unique id. Each sample
  /// will be calculated exactly from phase information.
  /// </summary>
  /// <param name="sampleRate"></param>
  /// <param name="generatorName"></param>
  /// <returns>unique id of the created source</returns>
  OscillatorSourceId createGeneratorSource(float sampleRate,
                                           WaveformGenerator generatorName);

  /// <summary>
  /// Creates a new wavetable oscillator source with a unique id.
  /// Each sample will be approximated from the given wavetable based on phase
  /// information and interpolation algorithm.
  /// </summary>
  /// <param name="sampleRate"></param>
  /// <param name="waveTable"></param>
  /// <returns>unique id of the created source</returns>
  OscillatorSourceId createWaveTableSource(float sampleRate,
                                           std::vector<float> waveTable);

  /// <summary>
  /// Removes oscillator source with the given <paramref name="sourceId"/>.
  /// One source belongs to one oscillator, so it can affect only one oscillator
  /// (all oscillators with a particular id to be exact).
  /// </summary>
  /// <param name="sourceId"></param>
  void removeOscillatorSource(OscillatorSourceId sourceId);

  /// <summary>
  /// Creates a new oscillator with the given source and adds it to the sound
  /// generation process.
  /// </summary>
  /// <param name="sourceId"></param>
  ...
  /// </summary>
  /// <param name="oscillatorId">id of the oscillator to set the source
  /// of</param> <param name="sourceId">id of the source to set</param>
...
private:
  ...
  /// </summary>
  std::vector<std::shared_ptr<synth::wavetable::SignalGenerator>>
      _signalGenerators;

  /// <summary>
  /// Each new oscillator source creation will assign this value as the source's
  /// unique id and then increment it.
  /// </summary>
  ...
  /// </summary>
  std::map<OscillatorSourceId,
           std::unique_ptr<synth::wavetable::IOscillatorSource>>
      _oscillatorSources;

  /// <summary>
  /// Each new oscillator creation will assign this value as the oscillator's
  /// unique id and then increment it.
  /// </summary>
