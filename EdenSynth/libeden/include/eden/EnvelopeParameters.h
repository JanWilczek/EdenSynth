...
enum class EnvelopeType { ADBDR, ADSR };
...
/// The exponential gain sounds linear to the human ear, since it is linear on
/// the dB scale.
/// </summary>
enum class EnvelopeSegmentCurve { Linear = 0, Exponential = 1 };
...
/// envelope. Such an envelope is appropriate for simulating piano-like envelope
/// when Decay2 time is very long. The break level parameter specifies at what
/// amplitude the envelope should pass from Decay1 to Decay2 segment.
/// </summary>
struct ADBDRParameters final : public EnvelopeParameters {
  ADBDRParameters() : EnvelopeParameters(EnvelopeType::ADBDR) {}

  ADBDRParameters(std::chrono::milliseconds _attackTime,
                  EnvelopeSegmentCurve _attackCurve,
                  std::chrono::milliseconds _decay1Time,
                  EnvelopeSegmentCurve _decay1Curve,
                  std::chrono::milliseconds _decay2Time,
                  EnvelopeSegmentCurve _decay2Curve,
                  std::chrono::milliseconds _releaseTime,
                  EnvelopeSegmentCurve _releaseCurve,
                  float _breakLevel)
      : EnvelopeParameters(EnvelopeType::ADBDR),
        attackTime(_attackTime),
        attackCurve(_attackCurve),
        decay1Time(_decay1Time),
        decay1Curve(_decay1Curve),
        decay2Time(_decay2Time),
        decay2Curve(_decay2Curve),
...
struct ADSRParameters final : public EnvelopeParameters {
  ADSRParameters() : EnvelopeParameters(EnvelopeType::ADSR) {}
