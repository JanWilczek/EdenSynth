...
/// <summary>
/// Class representing a synthesiser voice which can be thought of as a single
/// note on the keyboard. It contains all necessary elements of a typical
/// synthesizer:
...
public:
  ...
  /// <summary>
  /// Stops playing note. It should only be called if <c>startNote()</c> method
  ...
  /// <summary>
  /// Renders played sound given number of samples. Fills the buffer with zeros
  /// if not playing.
  /// </summary>
  ...
  /// <param name="pitchBendValue">pitch bend value as specified in the MIDI
  /// standard. <see cref="settings::Tuner::getPitchBendInSemitones(int)"> for
  /// predefined pitch bend values</param>
  void setPitchBend(int pitchBendValue);

  /// <returns>true if the voice is active (renders a note in
  /// <c>renderBlock()</c>), false otherwise</returns>
  bool isPlaying() const noexcept;

  /// <returns>true if the voice is currently rendering a particular note, false
  /// otherwise</returns>
  bool isPlayingNote(const int midiNoteNumber) const noexcept;

  /// <summary>
  /// Sets the expected length of processing block. Must be called at least once
  /// before first call to <c>renderBlock()</c>.
  /// </summary>
  /// <param name="samplesPerBlock"></param>
  void setBlockLength(unsigned samplesPerBlock);

  /// <summary>
  /// Makes the voice available for a new note. Typically should be called when
  /// the envelope's release has ended.
  /// </summary>
  void finalizeVoice();
...
private:
  ...
  /// <summary>
  /// Responsible for applying envelope to the signal. It holds the information
  /// about the current level of rendered samples.
  /// </summary>
  ...
  /// <summary>
  /// This information if necessary if a new note is pressed and a new voice has
  /// to be launched whilst the pitch wheel is being moved. Then it starts to
  /// play with this value as pitch bend value.
  /// </summary>
  int _lastPitchBendValue;

  /// <summary>
  /// Currently played note by this voice. -1 means that no note is being played
  /// and voice is free to play a new one.
  /// </summary>
  int _currentNote = -1;
