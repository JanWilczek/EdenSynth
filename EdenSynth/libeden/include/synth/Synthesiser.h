...
public:
  ...
  void processBlock(AudioBuffer& bufferToFill,
                    MidiBuffer& midiBuffer,
                    int numSamples);

  /// <summary>
  /// Fills the given audio buffer with samples from <paramref
  /// name="startSample"/> (including) to <paramref name="startSample"/> +
  /// <paramref name="numSamples"/> (excluding).
  /// </summary>
  /// <param name="bufferToFill"></param>
  /// <param name="midiBuffer"></param>
...
private:
  ...
  /// <summary>
  /// Changes internal state based on received MIDI message. E.g. starts playing
  /// a note after note on message.
  /// </summary>
  ...
  /// <summary>
  /// Adds voices given number of voices to the synthesiser. More voices means,
  /// that more notes can be played at once.
  /// </summary>
  ...
  /// <param name="midiNoteNumber"></param>
  /// <returns>voice that is playing given note, nullptr if there is no such
  /// voice</returns>
  Voice* getVoicePlayingNote(const int midiNoteNumber);

  /// <summary>
  /// Applies global volume to the buffer in range [<paramref
