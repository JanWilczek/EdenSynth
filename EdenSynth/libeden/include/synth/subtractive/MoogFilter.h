...
/// Based on an article "Oscillator and Filter Algorithms for Virtual Analog
...
class MoogFilter {
 public:
  ...
  /// <summary>
  /// Sets the cutoff frequency of the filter (frequency at which attenuation is
  /// 3 dB).
  /// </summary>
 ...
 private:
  ...
  /// <summary>
  /// Subsequent one pole filters. Signal taken from between them constitutes
  /// the output signal.
  /// </summary>
  ...
  /// <summary>
  /// Factors with which each signal output contributes to the final output.
  /// parameters <c>_a</c>, <c>_b</c> and <c>_d</c> may be used to create
  /// filters other than low-pass, but this possibility is not currently
  /// implemented.
  /// </summary>
  const float _a = 0.f;
  const float _b = 0.f;
