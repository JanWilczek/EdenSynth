...
namespace eden::synth::subtractive
{
	...
	public:
		...
		/// <summary>
		/// Apply filtering on <paramref name="audioChannel"/> in range [<paramref name="startSample"/>, <paramref name="startSample"/> + <paramref name="samplesToProcess"/>). 
		/// </summary>
		...
		/// <summary>
		/// Sets cutoff, i.e. number of partial to set the cutoff frequency at. 1 means that the cutoff frequency will be set at the fundamental frequency.
		/// </summary>
		/// <param name="cutoff">number of partial to set the cutoff frequency at</param>
		...
		/// <summary>
		/// Sets the extent to which cutoff frequency is affected by filter's envelope: <see cref="_contourAmount"/>.
		/// </summary>
		/// <param name="contourAmount">envelope scale in range [0, 1]</param>
		void setContourAmount(float contourAmount);

		/// <summary>
		/// Sets the attenuation strength in the passband of the filter. 12 dB per octave means gentle attenuation, 24 dB per octave means strong (steep) attenuation.
		/// </summary>
		...
		/// <summary>
		/// Sets pitch of the currently played note. Use it to have pitch- or key-follow functionality.
		/// </summary>
	...
	private:
		/// <summary>
		/// Sets the cutoff frequency in Hz of inner filter based on <see cref="_cutoff"/>, <see cref="_contourAmount"/>, <see cref="_pitch"/> and <see cref="_cutoffFrequencyEnvelope"/>.
		/// </summary>
		...
		/// <summary>
		/// Envelope of the filter's cutoff frequency. It's peak is always at <c>_cutoff</c> * <c>_pitch</c>.
		/// </summary>
