...
ADSR::ADSR(float sampleRate, ADSRParameters parameters)
    : _attack(sampleRate,
              ...
              1.0f),
      _decay(sampleRate,
             SegmentGainFactory::createSegmentGain(parameters.decayCurve),
      ...
      _release(sampleRate,
               ...
               parameters.sustainLevel,
               0.f) {
  _segments.insert(_segments.begin() + static_cast<int>(ADSRSegments::Attack),
                   &_attack);
  _segments.insert(_segments.begin() + static_cast<int>(ADSRSegments::Decay),
                   &_decay);
  _segments.insert(_segments.begin() + static_cast<int>(ADSRSegments::Sustain),
                   &_sustain);
  _segments.insert(_segments.begin() + static_cast<int>(ADSRSegments::Release),
                   &_release);
  _segments.insert(_segments.begin() + static_cast<int>(ADSRSegments::Silence),
                   &_silence);
}
...
void ADSR::setSegmentTime(ADSRSegments segment,
