...
{
	std::unique_ptr<Envelope> EnvelopeFactory::createEnvelope(float sampleRate, std::shared_ptr<EnvelopeParameters> parameters)
	{
	...
	std::unique_ptr<Envelope> EnvelopeFactory::createADBDREnvelope(float sampleRate, std::shared_ptr<ADBDRParameters> parameters)
	{
		return std::make_unique<ADBDR>(sampleRate, *parameters);
	}

	std::unique_ptr<Envelope> EnvelopeFactory::createADSREnvelope(float sampleRate, std::shared_ptr<ADSRParameters> parameters)
	{
