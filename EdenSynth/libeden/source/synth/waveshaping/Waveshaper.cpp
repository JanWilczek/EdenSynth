...
float Waveshaper::transform(float sample) {
  ...
  const auto index =
      sample * halfShapingFunctionLength + halfShapingFunctionLength;
...
void Waveshaper::setTransferFunction(std::vector<float> transferFunction) {
  ...
  _makeUpGainFactor =
      1.f / std::min(*std::max_element(_transferFunction.begin(),
                                       _transferFunction.end()),
                     *std::min_element(_transferFunction.begin(),
                                       _transferFunction.end()));
}
