...
void Synthesiser::processBlock(AudioBuffer& bufferToFill,
...
void Synthesiser::processBlock(AudioBuffer& bufferToFill,
  ...
  while (numSamples > 0) {
    ...
    const auto samplesToNextMidiMessage =
        midiIterator != midiBuffer.end()
            ? midiIterator->sampleTimeStamp - startSample
            : numSamples;
...
void Synthesiser::SynchronousVoiceRenderer::renderVoices(
    Synthesiser& synthesiser,
    AudioBuffer& outputBuffer,
    int startSample,
    int samplesToProcess) {
  for (auto i = 0u; i < NB_VOICES; ++i) {
    synthesiser._mixer[i] =
        synthesiser._voices[i]->renderBlock(samplesToProcess);
  }

  synthesiser._mixer.mixTo(outputBuffer, startSample, samplesToProcess);
}

void Synthesiser::AsynchronousVoiceRenderer::renderVoices(
    Synthesiser& synthesiser,
    AudioBuffer& outputBuffer,
    int startSample,
    int samplesToProcess) {
...
void Synthesiser::renderVoices(AudioBuffer& outputBuffer,
                               int startSample,
                               int samplesToProcess) {
  EDEN_ASSERT(samplesToProcess >= 0);

  _voiceRenderer->renderVoices(*this, outputBuffer, startSample,
                               samplesToProcess);
}
...
void Synthesiser::pitchBendChange(const int midiChannel,
...
void Synthesiser::addVoices(settings::Settings& settings,
                            unsigned numVoicesToAdd) {
...
Voice* Synthesiser::getFreeVoice() {
  const auto result = std::find_if_not(
      _voices.begin(), _voices.end(),
      [](std::unique_ptr<Voice>& voice) { return voice->isPlaying(); });
  return result != _voices.end() ? result->get() : nullptr;
}

Voice* Synthesiser::getVoicePlayingNote(const int midiNoteNumber) {
  const auto result =
      std::find_if(_voices.begin(), _voices.end(),
                   [&midiNoteNumber](std::unique_ptr<Voice>& voice) {
                     return voice->isPlayingNote(midiNoteNumber);
                   });
  return result != _voices.end() ? result->get() : nullptr;
}

void Synthesiser::applyVolume(AudioBuffer& bufferToFill,
                              int startSample,
                              int numSamples) {
  bufferToFill.forEachChannel([&](float* channel) {
    for (auto sample = startSample; sample < startSample + numSamples;
         ++sample) {
