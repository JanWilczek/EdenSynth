...
namespace eden::synth::subtractive
{
	...
	float OnePoleLowPassFilter::processSample(const float sample)
	{
		...
		const auto firstStageSample = sample * multiplyCoeff + _feedforwardSample * feedforwardCoeff;
	...
	{
		const auto omega_c = 2 * static_cast<float>(math_constants::PI) * _cutoffFrequency / _sampleRate;
		_g = 0.9892f * omega_c - 0.4342f * std::pow(omega_c, 2.f) + 0.1381f * std::pow(omega_c, 3.f) - 0.0202f * std::pow(omega_c, 4.f);
	}
}
