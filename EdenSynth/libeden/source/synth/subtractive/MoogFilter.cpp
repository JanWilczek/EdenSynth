...
namespace eden::synth::subtractive
{
	...
	void MoogFilter::setPassbandAttenuation(PassbandAttenuation passbandAttenuation)
	{
	...
	{
		const auto omega_c = 1 * static_cast<float>(math_constants::PI) * _cutoffFrequency / _sampleRate;
		_gRes = _resonance * (1.0029f + 0.0526f * omega_c - 0.0926f * std::pow(omega_c, 2.f) + 0.0218f * std::pow(omega_c, 3.f));
	}
}
