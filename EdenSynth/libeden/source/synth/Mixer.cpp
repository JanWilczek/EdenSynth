...
Mixer::Mixer(size_t nb_voices) : _renderedVoices(nb_voices) {}

const float*& Mixer::operator[](size_t voice) {
  return _renderedVoices[voice];
}

void Mixer::mixTo(AudioBuffer& outputBuffer,
                  int startSample,
                  int samplesToMix) {
  outputBuffer.forEachChannel(
      [this, &startSample, &samplesToMix](float* channel) {
