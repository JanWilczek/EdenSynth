...
void SignalGenerator::generateSignal(float* audioChannel,
                                     int startSample,
                                     int samplesToGenerate) {
  for (int sample = startSample; sample < startSample + samplesToGenerate;
       ++sample) {
...
void SignalGenerator::setOscillatorSource(
    OscillatorId oscillatorId,
    std::unique_ptr<IOscillatorSource> source) {
  _oscillators[oscillatorId].setSource(std::move(source));
}

void SignalGenerator::setOctaveTransposition(OscillatorId oscillatorId,
                                             int octaveShift) {
  _oscillators[oscillatorId].setOctaveTransposition(octaveShift);
}

void SignalGenerator::setSemitoneTransposition(OscillatorId oscillatorId,
                                               int semitoneShift) {
  _oscillators[oscillatorId].setSemitoneTransposition(semitoneShift);
}

void SignalGenerator::setCentTransposition(OscillatorId oscillatorId,
                                           int centShift) {
  _oscillators[oscillatorId].setCentTransposition(centShift);
}

void SignalGenerator::setOscillatorVolume(OscillatorId oscillatorId,
                                          float volume) {
