...
void SynthOscillator::setSource(
    std::unique_ptr<IOscillatorSource> oscillatorSource) {
  _oscillatorSource = std::move(oscillatorSource);
  setOscillatorPitch();
}
...
float SynthOscillator::pitch() {
  const auto transpositionInCents = static_cast<float>(_octaveShift) * 1200.f +
                                    static_cast<float>(_semitoneShift) * 100.f +
                                    static_cast<float>(_centShift);
