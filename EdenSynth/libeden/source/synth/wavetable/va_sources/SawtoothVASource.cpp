...
void SawtoothVASource::setPitch(float pitch) {
  ...
  // phase in getSample() runs from 0 to 1 in the "unipolar sawtooth"
  // generation, hence phase shift in range [0, 1] directly corresponds to shift
  // in phase relative to the whole period (for example, 0.5 means shift by pi
  // in phase)
  _phase = _phaseShift;
}
