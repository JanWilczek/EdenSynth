...
void GeneratorSettings::registerSignalGenerator(
...
OscillatorSourceId GeneratorSettings::createGeneratorSource(
    float sampleRate,
...
OscillatorSourceId GeneratorSettings::createWaveTableSource(
    float sampleRate,
...
OscillatorId GeneratorSettings::addOscillator(OscillatorSourceId sourceId) {
  ...
  for (auto generator : _signalGenerators) {
    generator->addOscillator(
        synth::wavetable::SynthOscillator(id, _oscillatorSources[id]->clone()));
  }
...
void GeneratorSettings::setOscillatorSource(OscillatorId oscillatorId,
  ...
  for (auto generator : _signalGenerators) {
    generator->setOscillatorSource(oscillatorId,
                                   _oscillatorSources[sourceId]->clone());
  }
}

void GeneratorSettings::setOctaveTransposition(OscillatorId oscillatorId,
                                               int octaveShift) {
  for (auto generator : _signalGenerators) {
...
void GeneratorSettings::setSemitoneTransposition(OscillatorId oscillatorId,
                                                 int semitoneShift) {
  for (auto generator : _signalGenerators) {
...
void GeneratorSettings::setCentTransposition(OscillatorId oscillatorId,
                                             int centShift) {
  for (auto generator : _signalGenerators) {
...
void GeneratorSettings::setOscillatorVolume(OscillatorId oscillatorId,
                                            float volume) {
  for (auto generator : _signalGenerators) {
